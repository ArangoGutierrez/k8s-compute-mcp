...
IMAGE_NAME = $(REGISTRY)/k8s-compute-mcp
endif
...
all: check test build ## Run all checks, tests, and build
...
server: cmd-server ## Build server binary (convenience target)
...
test-integration: build ## Run integration tests (requires K8s cluster)
...
DOCKERFILE ?= $(CURDIR)/deployment/Dockerfile
...
.build-image:
	$(DOCKER) build \
		-f $(CURDIR)/deployment/Dockerfile.build \
		-t $(BUILDIMAGE) \
...
dist: clean ## Build release binaries for all platforms
	@$(MKDIR) -p $(DIST_DIR)
	CGO_ENABLED=0 GOOS=linux GOARCH=amd64 $(MAKE) cmd-server PREFIX=$(DIST_DIR)/linux-amd64
	CGO_ENABLED=0 GOOS=linux GOARCH=arm64 $(MAKE) cmd-server PREFIX=$(DIST_DIR)/linux-arm64
	CGO_ENABLED=0 GOOS=darwin GOARCH=amd64 $(MAKE) cmd-server PREFIX=$(DIST_DIR)/darwin-amd64
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 $(MAKE) cmd-server PREFIX=$(DIST_DIR)/darwin-arm64
	@echo "✓ Release binaries built in $(DIST_DIR)"

dist-checksums: dist ## Generate checksums for release binaries
	cd $(DIST_DIR) && \
		find . -type f -name 'server' -exec sha256sum {} \; > SHA256SUMS
...
.PHONY: test-e2e test-e2e-setup test-e2e-teardown
...
test-e2e-setup: ## Setup Kind cluster for E2E tests
	@if ! kind get clusters 2>/dev/null | grep -q '^e2e-compute-mcp$$'; then \
		echo "Creating Kind cluster 'e2e-compute-mcp'..."; \
		kind create cluster --name e2e-compute-mcp --wait 120s; \
	else \
		echo "Kind cluster 'e2e-compute-mcp' already exists"; \
	fi
	@echo "✓ E2E environment ready"

test-e2e-teardown: ## Teardown Kind cluster
	kind delete cluster --name e2e-compute-mcp
